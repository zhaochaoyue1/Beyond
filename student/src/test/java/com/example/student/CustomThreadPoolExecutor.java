...
*/
     ...
     * start com.example.spi.SpiInterface new thread with the given command as its first
     * task.  The call to addWorker atomically checks runState and
     * workerCount, and so prevents false alarms that would add
     * threads when it shouldn't, by returning false.
     *
     * 2. If com.example.spi.SpiInterface task can be successfully queued, then we still need
     * to double-check whether we should have added com.example.spi.SpiInterface thread
     * (because existing ones died since last checking) or that
     * the pool shut down since entry into this method. So we
     * recheck state and if necessary roll back the enqueuing if
     * stopped, or start com.example.spi.SpiInterface new thread if there are none.
     *
     * 3. If we cannot queue task, then we try to add com.example.spi.SpiInterface new
